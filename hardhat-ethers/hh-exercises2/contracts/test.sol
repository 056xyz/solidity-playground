...
contract Staking is ReentrancyGuard { 
    ...
    function withdraw() public nonReentrant { 
        ...
        require(block.timestamp >= lastStakeTimestamp[msg.sender] + LOCK_PERIOD, "Lock period not over"); 
        uint256 amount = stakes[msg.sender]; 
        uint256 reward = calculateReward(msg.sender); stakes[msg.sender] = 0; 
        rewards[msg.sender] = 0; // Transfers should be last payable(msg.sender).transfer(amount); 
        payable(msg.sender).transfer(reward); emit Withdrawn(msg.sender, amount); 
